    ...
    subscription::channel(
        ...
        |mut output| async move {
            ...
            loop {
                match &mut state {
                    State::Disconnected => {
                        let order_book = diff_book_depth_stream(&pair, 1000);
                        ...
                        let mut web_socket = WebSockets::new(
                            move |events: binance::ws_model::DepthOrderBookEvent| {
                                ...
                                let _ = output_clone.try_send(WsMessage::Book(WsEvent::Message(
                                    OrderBookDetails {
                                        sym: symbol,
                                        bids: b,
                                        asks: a,
                        ...
                        match web_socket.connect(&order_book).await {
                            Ok(()) => {
                                ...
                                let _ = output.send(WsMessage::Book(WsEvent::Connected(tx))).await;
                                state = State::Connected(web_socket, rx);
                            }
                            Err(e) => {
